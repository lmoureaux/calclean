...
const int ETA_DIVS = 60;
/// The number of bins in @f$\eta@f$.
const int CENTER_IETA = 15;
...
int main(int argc, char *argv[])
{
  ...
  for (int numhot = 0; numhot <= 15; ++numhot) {
...
results::results(const config &c, calo::towerset &tset)
{
  if (hit::all.size() == 0) {
    ...
    for (unsigned long entry = 0; entry < tset.entries(); ++entry) {
      ...
      for (calo::towerset::iterator it = tset.begin(&ee); it != end; ++it) {
        hit h;
        h.eta = it->eta();
        h.phi = it->phi();
        h.emenergy = it->emenergy();
        hit::all.push_back(h);
      }
    }
  }
  ...
  // For each eta
  for (int ieta = -ETA_DIVS / 2; ieta < ETA_DIVS / 2; ++ieta) {
    if (ieta >= -CENTER_IETA && ieta < CENTER_IETA) {
      // In EB
      _energies[ieta + ETA_DIVS / 2] = -1;
      continue;
    }
    ...
    int status = rootfinder.Solve(cost, 0, 30, 100, 1e-4, 1e-4);
    if (status != 1) {
      std::cerr << "Warning: Optimization failled for ieta=" << ieta
                << ", numhot=" << c.numhot() << std::endl;
      _energies[ieta + ETA_DIVS / 2] = -1;
      continue;
    }
...
TH1D *make_histogram(int ieta, double energy)
{
  ...
  for (std::vector<hit>::iterator it = hit::all.begin(); it != end; ++it) {
    if (it->emenergy > energy && ieta == std::floor(it->eta / 0.1)) {
      hits->Fill(it->phi);
    }
  }
...
std::vector<int> cost_function::getignored(double energy) const
{
  ...
  while (s.pvalue < pvalue && ignore.size() < PHI_DIVS) {
